...
Студенческий проект. Учёт личных финансов с аналитикой и ML-прогнозированием баланса.
...
Go backend — единственная точка входа. ML сервис не имеет доступа к БД.
...
- ML сервис НЕ обращается к БД, НЕ парсит CSV, НЕ содержит бизнес-логики
- CSV парсится только в Go
...
```
TransactionService.UploadCSV(file io.Reader) (int, AppError)
...
Приоритет: `csv/` и `service/` — критическая логика.
- `csv/parser_test.go` — белый ящик, граничные случаи
