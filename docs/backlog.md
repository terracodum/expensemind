...
### parser/ — фабрика парсеров
- Добавить `ParserFactory` в `parser/` — принимает расширение файла (`.csv`, `.pdf`), возвращает нужный `Parser`
- Сервис работает только с фабрикой, не знает про конкретные парсеры
